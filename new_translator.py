...
from new_isa import Opcode as Opcode
from new_isa import write_bin_code, write_json_code
...
def buildAST(tokens: list[str]):
    ...
    parseUnit.driveIn()
    for token in tokens:
        # print(parseUnit.getCurrentTerm())
        # print(token)
        # print(parseUnit.nesting_level)
        # print()
...
def write_code(filename: str, memory: list, program: list):
    """Записать машинный код в файл."""
    with open(filename, "w", encoding="utf-8") as file:
        file.write(json.dumps({'data': memory, 'code': [{'index': index, 'instr': instr} for index, instr in enumerate(program)]}, indent=4))
...
def main(args):
    assert len(args) == 3, \
        "Wrong arguments: new_translator.py <input_file> <target_json_file> <target_bin_file>"
    ...
    print(program)
    # print(data)
    write_code(target_json, memory, program)
